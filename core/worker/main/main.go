...
var (
	rootCmd = &cobra.Command{
		Use:        "worker",
		Short:      "worker for mini-ray",
		SuggestFor: []string{"worker"},
		Run:        execute,
	}
...
func execute(cmd *cobra.Command, args []string) {
	...
	lg.Info("--__V__-- worker starting", zap.String("version", "0.0.1"))
	...
	server := worker.NewWorker(ctx, lg, workerPool, listenAddr, advertiseAddr, opts...)
	server.Start()
