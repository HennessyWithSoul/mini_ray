...
var (
	_ common.Connection  = (*connection)(nil)
	_ common.ReadHandler = (*connection)(nil)
)
...
func newConnection(cm *connectionManager, srv *GCS, id uint64, remote string, gc gnet.Conn, lg *zap.Logger) *connection {
	return &connection{
		cm:            cm,
		srv:           srv,
		id:            id,
...
func (c *connection) OnPacket(header *codec.Header, msg proto.Message) error {
	return c.cm.OnPacket(c, header, msg)
}
...
func (c *connection) Loop() {
	for {
		// c.cm.pool.Submit(func() {
		// 	c.readLoop(context.Ba	ckground())
		// })
		//c.Establish()
		select {
...
func (c *connection) Establish() {
	...
	payload, err := codec.EncodePayload(
		...
		&pb.EstablishReq{Addr: c.cm.advertiseAddr},
	)
	if err != nil {
